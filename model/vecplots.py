...
def generate_vehicle(nexus):
    ...
    if hasattr(nexus,'Surrogate_Data'):
        ...
        Exception("Please name Surrogate_Data() structure as \'surrogate_data\' ")
    ...
    aero_geom, A_ref, span_vals, total_mass=get_aero_geometry(vec, material, pts, aerofoil, geom, payload)
...
def get_aero_geometry(vec, material, all_points, aerofoil, geom, payload):
...
#    plt.show()
    plt.savefig('./stevens/save'+str(time.time())+'.png')
...
#    print init_vec.aerofoil
    ...
    with open('./stevens/steven.csv','rb') as csvfile:
        inread  = csv.reader(csvfile, delimiter=',')
        for row in inread:
            row = map(np.float64,row)
...
#            quit()
            ...
            init_vec.psl             = np.array([0., row[1]])
            init_vec.rcp             = np.array([1., row[2], row[3]]) #[-1] is tip taper
            init_vec.sqc             = np.array([row[4], row[5]])
            init_vec.do              = np.array([row[6], row[7]])
            init_vec.tw              = np.array([row[8], row[9]]) # just root and tip
...
#            init_vec.rcp             = np.array([1., .5, .1]) #[-1] is tip taper
##            init_vec.psl             = np.array([0., 0.35])
#            init_vec.sqc             = np.array([40., 25.])
#            init_vec.do              = np.array([5., 0.])
