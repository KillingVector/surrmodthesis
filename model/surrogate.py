...
###################################         LOADING AND PLOTTING FROM FILES
    ...
    def ck_from_file(self,nexus,filename):
        # unpack data
...
#        matplotlib inline
        ...
        sampsize = np.shape(data)[0]
        Xc = np.atleast_2d(data[:,0:len(names)])
        yc = data[:,len(names)]

        ye = []
        Xe = []
        ct = 0
        for item in data[:,len(names)+1]:
            if not item == 0 and not item == 0.:
                ye.append(item)
            else:
                ct = ct+1
        Xe = data[ct:sampsize+1,0:len(names)]
        yck= data[:,len(names)+2]
        ymass=data[:,len(names)+3]
        ...
        print 'Training kriging model for mass...'
        km          = kriging(Xc,ymass)
        km.train()
        self.model0 = km
        print 'Mass model complete'

        print 'Trainging co-kriging model...'
        ck          = ckrig(Xc,yc,Xe,ye)
        ck.train()
        self.modelck0   = ck
    ...
    def plot_from_file(self,nexus,filename):
        # unpack data
        from mpl_toolkits import mplot3d
#        matplotlib inline
        import matplotlib.pyplot as plt
        ...
        ax0.plot_trisurf(Xc[:,0],Xc[:,1],yc,edgecolor='b',linewidth=.25,alpha=0.0)
        ax0.scatter(Xe[:,0],Xe[:,1],ye,c='r',marker='o',s=10)
        ax0.set_xlabel(names[0])
        ax0.set_ylabel(names[1])
        ax0.set_zlabel('-L/D')
        ax0.set_title('Surface: cheap, Points: Exp')
        plt.show()


        fig1 = plt.figure(figsize=(8,8))
        ax1 = plt.axes(projection='3d')
        ax1.plot_trisurf(Xc[:,0],Xc[:,1],yck,cmap='viridis', edgecolor='none',alpha=0.8)
        ax1.set_xlabel(names[0])
        ax1.set_ylabel(names[1])
        ax1.set_zlabel('-L/D')
        ax1.set_title('Co-kriging surface')
        plt.show()


# MAKE THESE SUBPLOTS
# https://stackoverflow.com/questions/31559262/set-size-of-matplotlib-figure-with-3d-subplots
        fig3 = plt.figure(figsize=(8,8))
        ax3 = plt.axes(projection='3d')
        ax3.scatter(Xc[:,0],Xc[:,1],diff)#,cmap='viridis', edgecolor='none',alpha=0.8)
        ax3.set_xlabel(names[0])
        ax3.set_ylabel(names[1])
        ax3.set_zlabel('Difference in -L/D') 
        ax3.set_title('Difference, y_ck-y_c')
        plt.show()

        fig4 = plt.figure(figsize=(8,8))
        ax4 = plt.axes(projection='3d')
        ax4.plot_trisurf(Xc[:,0],Xc[:,1],yc,edgecolor='k',linewidth=.25,alpha=0.0)
        ax4.plot_trisurf(Xc[:,0],Xc[:,1],yck,edgecolor='r',linewidth=.25,alpha=0.0)
        ax4.set_xlabel(names[0])
        ax4.set_ylabel(names[1])
        ax4.set_zlabel('-L/D')
        ax4.set_title('Red: cokrig, Blue: cheap') # legend is apparently a myth for surface
#        ax4.legend()
        plt.show()
